...
Inputs:
    - mileage (float): Fuel economy of the truck (kWh / mile)
    - average_VMT (float): Average vehicle miles traveled per year, used to scale the VMT distribution
...
"""
def get_vehicle_model_results(mileage, m_payload_lb, average_VMT, truck_name, battery_chemistry, e_bat, m_truck_max_lb=82000):

    # Read in parameters for the given truck type
    parameters = data_collection_tools_messy.read_parameters(truck_params = truck_name, vmt_params = 'daycab_vmt_vius_2021', run='messy_middle', truck_type='EV')
...
Inputs:
    - mileage_mpg (float): Fuel economy of the truck (miles / gal)
...
"""
def get_vehicle_model_results_diesel(mileage_mpg, m_payload_lb, average_VMT, truck_type='diesel_daycab', m_truck_max_lb=80000):

    # Read in parameters for the given truck type
    parameters = data_collection_tools_messy.read_parameters(truck_params=truck_type, vmt_params='daycab_vmt_vius_2021', run='messy_middle', truck_type='diesel')
    ...
    vehicle_model_results_dict = {
        'Fuel economy (miles/gal)': mileage_mpg,
...
Inputs:
    ...
    - m_truck_max_lb (float): Maximum allowable GVW of the truck (82000lb for EVs in California)
"""
def evaluate_emissions(mileage, payload_lb, grid_emission_intensity, average_VMT, e_bat, battery_chemistry, truck_name, grid_emission_intensity_year=2022, truck_simulation_start_year=2026):
    
    # Evaluate parameters and vehicle model results for the given payload
    parameters, vehicle_model_results_dict = get_vehicle_model_results(mileage, payload_lb, average_VMT, truck_name=truck_name, e_bat=e_bat, battery_chemistry=battery_chemistry)
...
"""
def evaluate_costs(mileage, payload_lb, electricity_charge, demand_charge, average_VMT, charging_power, e_bat, battery_chemistry='NMC', truck_name=None, vehicle_purchase_price=None):
...
Inputs:
    - mileage_mpg (float): Fuel economy of the truck (miles / gal)
    - payload_lb (float): Payload carried by the truck, in lb
    - m_truck_max_lb (float): Maximum allowable GVW of the truck (80000lb for diesel)
    - scenario (string): Time scenario (Present, Mid term or Long term)
"""
def evaluate_costs_diesel(mileage_mpg, payload_lb, diesel_price=3.67, average_VMT=85000, m_truck_max_lb=80000, vehicle_purchase_price=None, truck_type='diesel_daycab'):
    
    # Evaluate parameters and vehicle model results for the given payload
    parameters, vehicle_model_results_dict = get_vehicle_model_results_diesel(mileage_mpg, payload_lb, average_VMT, truck_type=truck_type)
    
    # Read in costing data for the diesel truck
    truck_cost_data = data_collection_tools_messy.read_truck_cost_data(truck_type='diesel')
