...
def evaluate_linear_params_ev():
    ...
    evaluated_linear_params_df = pd.DataFrame(columns=['Truck', 'Linear Slope (kWh/mile/lb)', 'Y Intercept (kWh/mile)', 'Predicted Payload (lb)', 'Predicted GVW (lb)'])
    for truck_name in drivecycles:
        for drivecycle in drivecycles[truck_name]:
            ...
            slope_per_kg, y_intercept_payload, kwh_per_mile_ave_payload = truck_model_tools.truck_model(parameters).get_linear_energy_economy_coefs(df_drivecycle, battery_params_dict['Roundtrip efficiency'], battery_params_dict['Energy density (kWh/ton)'], NACFE_results['Battery capacity (kWh)'])
            ...
            m_bat = NACFE_results['Battery capacity (kWh)'] * KG_PER_TON / battery_params_dict['Energy density (kWh/ton)']
            predicted_gvw = predicted_payload + (parameters.m_truck_no_bat + m_bat) / KG_PER_LB
            
            result = {
                'Truck': [make_title_string(truck_name)],
                'Linear Slope (kWh/mile/lb)': [slope_per_kiloton],
                'Y Intercept (kWh/mile)': [y_intercept_payload],
